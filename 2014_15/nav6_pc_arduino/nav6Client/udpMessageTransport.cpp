...
udpMessageTransport::udpMessageTransport(const char* ipAddress, unsigned short port)
{
...
void udpMessageTransport::send(BaseMessageClass &message)
{
   ...
   int bytes_sent = sendto(sock, message.message, message.messageSize(), 0,(struct sockaddr*)&sa, sizeof sa);
   if (bytes_sent < 0)
   {
      exit(EXIT_FAILURE);
   }
...
int udpMessageTransport::receive(unsigned char* buffer, int length)
{
   ...
   int size = recvfrom(sock, buffer, length, MSG_DONTWAIT, &tempAddr,  &len);
   if (size>0)
   {
      return size;
   }
