...
void udpServerMessageTransport::send(BaseMessageClass &message)
{
   ...
   if (rxConnectionValid)
   {
      try
      {
         bytes_sent = sendto(sock, message.message, message.messageSize(), 0, &rxConnection, sizeof rxConnection);
      }
      catch (int e)
      {
...
{
   unsigned int len = sizeof rxConnection;
   int size = recvfrom(sock, buffer, length, 0, &rxConnection,  &len);
   if (size>0)
   {
