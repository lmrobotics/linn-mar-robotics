 ...
 */
const float liftWheelDia=2.0; // lift wheel diameter
...
task main()
{
        ...
        //basket in flat position, servo value to be tweaked
        servo[servoClaw]=150;

        //arm lows to floor level
        armDown(20,0.5);

        //move forward to pickup ring, then raise basket
        Forward(30,6);
        servo[servoClaw]=58;
        armUp(50,1);

        //move back, then move forward and turn
        Backward(30,15);
        LeftTF(80,400); //degree value based on experiment

        //The following code decide if IR beacon is on current
        //column, if not, move to the next column, default to middle col
        if (isIRHere()) //Is IR on first column
					{
						LeftTOS(70,90); //turn left 90 degree if so
						irColumn=left;
					}
				else
					{
						Forward(35,19); //move to the middle column
						if(isIRHere())
							{
								LeftTOS(70,90); //turn left 90 degree if on middle
								irColumn=middle;
							}
						else
							{
								Forward(35,20); //move to the next column
								if(isIRHere())
									{
										LeftTOS(70,90);
										irColumn=right;
									...
									{
										Backward(35,19); //default to the middle column
										LeftTOS(70,90);
										irColumn=middle;
									}
							}
				...
				//now that the robot is facing the column where the IR beacon is on
				// raise arm and move sensor to position, move forward to the line
				if (irColumn==left)
				{
        	//distance to be tweaked
					Forward(50,40);
        	if ((SensorValue[LightL]>30)||(SensorValue[LightR]>30))
        		{
        			nxtDisplayCenteredTextLine(1,"%d",irColumn);
        			wait10Msec(200);
        			//LineFollow(motorPowerLineFollow);
        		}
        	else
        		{
        			//swing left and right until fining the line
      	...
      	if (irColumn==right)
      		...
      		//score ring, back 10 inches, stop
      		Stopbot();
        	wait1Msec(WWT);
          servo[servoMarm]=20;
     		  servo[servoClaw]=100;
...
void armUp(int motorPower, float heightInInches) //move arm up or down
{
    ...
    int armEncoderTarget = floor((revolutionPerTurn*heightInInches)/liftWheelDia);
