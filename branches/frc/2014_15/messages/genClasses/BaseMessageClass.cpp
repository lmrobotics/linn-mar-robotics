...
void BaseMessageClass::putFixed(fixed val, int msgOffset)
{
   int32 fixed = (int32)(val * 65536.0f);
   putInt32(fixed, msgOffset);
}

fixed BaseMessageClass::getFixed(int msgOffset)
{
   int32 fixed = getInt32(msgOffset);
   fixed val = float(fixed)/65536.0f;
   return val;
}
/** ***************************************************************************
...
void BaseMessageClass::putFloat (float val, int msgOffset)
{
   ...
   putInt32(mantissa*2147483647.0, msgOffset+2);
}
